...
VisibilityPolygon.compute = function (position: number[], segments: any) {
  var bounded = [];
  var minX = position[0];
  var minY = position[1];
  var maxX = position[0];
...
VisibilityPolygon.computeViewport = function (
  position: number[],
  segments: any,
  viewportMinCorner: any,
  viewportMaxCorner: any
) {
  var brokenSegments = [];
  var viewport = [
...
VisibilityPolygon.inPolygon = function (position: number[], polygon: any) {
  var val = polygon[0][0];
...
VisibilityPolygon.convertToSegments = function (polygons: any) {
  var segments = [];
...
VisibilityPolygon.breakIntersections = function (segments: any) {
  var output = [];
  for (var i = 0; i < segments.length; ++i) {
    var intersections = [];
...
VisibilityPolygon.equal = function (a: any, b: any) {
  if (
...
VisibilityPolygon.parent = function (index: number) {
  return Math.floor((index - 1) / 2);
};

VisibilityPolygon.child = function (index: number) {
  return 2 * index + 1;
};

VisibilityPolygon.angle2 = function (a: any, b: any, c: any) {
...
VisibilityPolygon.sortPoints = function (position: number[], segments: any) {
...
VisibilityPolygon.angle = function (a: number[], b: number[]) {
  return (Math.atan2(b[1] - a[1], b[0] - a[0]) * 180) / Math.PI;
};

VisibilityPolygon.intersectLines = function (
  a1: number[],
  a2: number[],
  b1: number[],
  b2: number[]
...
VisibilityPolygon.distance = function (a: number[], b: number[]) {
  var dx = a[0] - b[0];
  var dy = a[1] - b[1];
  return dx * dx + dy * dy;
};

VisibilityPolygon.isOnSegment = function (
  xi: number,
  yi: number,
  xj: number,
  yj: number,
...
VisibilityPolygon.computeDirection = function (
  xi: number,
  yi: number,
  xj: number,
  yj: number,
