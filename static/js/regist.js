function regist() {
  if (active) {
    ...
    if (password.value === rpassword.value) {
      errorPass.innerHTML = "";
      window
        .fetch("http://" + window.location.host + "/regist", {
          method: "POST",
          headers: {
      ...
      errorPass.innerHTML = "パスワードが一致しません";
    }
  }
}
let active = false;
...
function errorName() {
  ...
  input_name.style.border = "1px solid #FF0000";
}
window.addEventListener("DOMContentLoaded", function () {
  ...
  input_pass.addEventListener("blur", function () {
    if (this.value) {
      this.style.border = "1px solid #888888";
    } else {
      this.style.border = "1px solid #FF0000";
